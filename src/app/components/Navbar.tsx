...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (aboutSection) {
        ...
        if (rect.top > window.innerHeight * 0.6) setActiveSection('');
      }
    };
  ...
  useEffect(() => {
    ...
    if (activeEl && navEl && activeSection !== '') {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-purple-950/90 backdrop-blur-md border-b border-white/5 shadow-xl shadow-black/30' : 'bg-transparent'}`}>
        <div className="w-full px-6 h-16 flex items-center justify-between">
          ...
          <nav ref={navRef} className="hidden md:flex items-center gap-1 relative">
            ...
            {NAV_LINKS.map((link, i) => {
              ...
              return (
                <a key={link.href}
                  ...
                  className={`relative px-4 py-2 text-sm font-semibold rounded-full transition-colors duration-200 ${isActive ? 'text-white' : 'text-white/60 hover:text-white'}`}
                >
                  {link.label}
                </a>
              );
            ...
            <div className="flex items-center gap-3 ml-4 pl-4 border-l border-white/20">
              <a href="https://tiktok.com/@purplehope.ca" target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-white transition-colors duration-200"><FaTiktok size={18} /></a>
              <a href="https://instagram.com/purplehope.ca" target="_blank" rel="noopener noreferrer" className="text-white/60 hover:text-white transition-colors duration-200"><FaInstagram size={18} /></a>
          ...
          {/* Hamburger */}
          <button onClick={() => setMenuOpen((v) => !v)} aria-label="Toggle menu" className="md:hidden relative w-10 h-10 flex flex-col items-center justify-center gap-[5px]">
      ...
      <div className="fixed inset-0 z-40 md:hidden pointer-events-none">
        <div className={`absolute inset-0 bg-purple-950/80 backdrop-blur-xl transition-opacity duration-500 ${menuOpen ? 'opacity-100' : 'opacity-0'}`} />
        <div className={`absolute -top-32 -left-32 w-96 h-96 bg-purple-700/20 rounded-full blur-3xl transition-opacity duration-500 ${menuOpen ? 'opacity-100' : 'opacity-0'}`} />
        <div className={`absolute -bottom-32 -right-32 w-96 h-96 bg-blue-900/20 rounded-full blur-3xl transition-opacity duration-500 ${menuOpen ? 'opacity-100' : 'opacity-0'}`} />

        <nav className={`relative z-10 flex flex-col items-center justify-center h-full gap-2 transition-opacity duration-500 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          ...
          {NAV_LINKS.map((link, i) => (
            <a key={link.href} href={link.href}
              onClick={(e) => { e.preventDefault(); handleNavClick(link.href); }}
              className={`relative text-5xl font-bold font-serif text-white/80 hover:text-white transition-all duration-500 group ${menuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          ...
          <div className={`flex items-center gap-5 mt-8 transition-all duration-500 ${menuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
